...
ANOVA (**AN**alysis **O**f **VA**riance) helps us determine whether the means of multiple groups are different. Instead of comparing means directly (as in t-tests), ANOVA compares the variances to infer differences in means.

We assume that the the variance within each group is the same across all groups.
...
, and the probability of obtaining this F statistic (the p-value).
