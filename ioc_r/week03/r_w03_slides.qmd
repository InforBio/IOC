...
The output remains a data frame.
...
The output was "simplified" to a vector.
:::
...
merge(x = df1, y = df2, by = "id", all.y = TRUE) # right join
...
Use `[` `]` to subset a list, with name or (logical) index.
