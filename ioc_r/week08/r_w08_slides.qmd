...
mutate(iris, sepal_len_mm = Sepal.Length * 100, petal_len_mm = Petal.Length * 100) |>
  head(2)
```
...
```{r}
# combine conditions
iris |> filter(Sepal.Length > 6.8 & Species == "versicolor")
...
iris <- mutate(iris, index = seq_len(nrow(iris)))
...
```{r}
iris |> group_by(Species) |> slice(2) # the 2nd row of each species
iris |> group_by(Species) |> slice(5, 3) # the 5th and the 3rd rows of each species
```
