...
gene_info <- tibble(
  gene = paste0("gene", LETTERS[1:3]),
  description = c("growth regulation", "stress response", NA)
)
tidy_data_annot <- left_join(tidy_data, gene_info, by = "gene")
tidy_data_annot
```
...
tidy_data_annot |> drop_na()
# drop rows with NA in specified column(s)
tidy_data_annot |> drop_na(description)
```
