...
- Can take 0, 1 or more paramters (arguments) as input
...
- Example: rank gene based on log2FC and p-value using weighted sum
...
weighted_gene_score <- function(
  ...
  # We use the negative log10 of the p-value because lower p-values are better
  # use the absolute value of log2 fold change to avoid negative score 
...
```r
?apply
apply(X, MARGIN, FUN, ..., simplified = TRUE)
```
...
```{r}
gene_expr <- matrix(c(1, 2, 3, 2, 2, 3, 3, 3, 3), nrow = 3)
gene_expr
```
...
how many value is bigger than 2 in each column?
:::
