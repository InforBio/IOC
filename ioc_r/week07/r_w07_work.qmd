...
3. With the help of the `order()` function,
...
de_res[de_res$rank_padj %in% 1:3, ] # the top 3 adjusted p-value
```

4. With the help of the `order()` function,
rank genes based on the weighted sum of log2 fold change and -log10 adjusted p-value,
...
During the exercise, we've created a boxplot with p-value shown using the {`ggsignif`} package function.
We can also annotate the level of significance (NS, *, **, ***).
...
- From the differential expression results ([toy_DEanalysis.csv](../exos_data/toy_DEanalysis.csv)):
  - create a new column call `signif`, which shows
    - "***" for adjusted p-value < 0.001,
    - "**" for adjusted p-value < 0.01,
    - "*" for adjusted p-value < 0.05,
    - "ns" for adjusted p-value >= 0.05;
...
- Apply your function for a list of any genes from the analysis results.
...
target_genes <- de_res[order(de_res$padj), ][1:3, c("gene_name", "padj")]
target_genes$gene_name[1] # the 1st gene


# data frame for LOH1 boxplot
my_gene <- "PIR3"

gg_df <- data.frame(
  expr_val = c(
    unlist(counts[counts$Feature == my_gene, paste0("WT.", 1:10)]),
    unlist(counts[counts$Feature == my_gene, paste0("SET1.", 1:10)])
  ),
...
ggplot(gg_df, aes(x = group, y = expr_val)) +
  ...
  geom_signif(
    ...
    annotations = target_genes$signif[target_genes$gene_name == my_gene]
  ) +
  theme_bw()
