...
# Order by adjusted pvalue and extract the first 3 gene names
...
# alternative
# gg_df <- as.data.frame(
#   t(counts[counts$Feature == "LOH1", paste0(rep(c("WT.", "SET1."), each = 10), 1:10)])
# )
# gg_df$group <- rep(c("WT", "SET1"), each = 10)
```
...
p <- ggplot(gg_df, aes(x = group, y = expr_val)) +
  ...
  labs(x = NULL, y = "Expression Counts", title = "LOH1") +
  theme_bw()
...
Please install the [ggsignif](https://const-ae.github.io/ggsignif/) package.
We will use the function `geom_signif()` to add a layer to show p-value on the figure with a horizontal bar.

```{r}
library(ggsignif)
p_table <- de_res[order(de_res$padj), ][1:3, c("gene_name", "padj")]

p + geom_signif(
  comparisons = list(c("WT", "SET1")),
  annotations = p_table$padj[p_table$gene_name == "LOH1"], digits = 3 # show p-value as is
)

# improve p-value annotation
p <- p + geom_signif(
  comparisons = list(c("WT", "SET1")),
  annotations = signif(p_table$padj[p_table$gene_name == "LOH1"], digits = 3) # round p-value
)
...
draw_boxplot <- function(gene_i) {
  ...
  p <- ggplot(gg_df, aes(x = group, y = expr_val)) +
    ...
    geom_signif(
      comparisons = list(c("WT", "SET1")),
      annotations = signif(p_table$padj[p_table$gene_name == gene_i], digits = 3)
    ) +
    theme_bw()
