...
::: {.column .fragment .content-smaller}
...
named_mat <- mat
colnames(named_mat) <- c("col1", "col2", "col3")
colnames(named_mat) <- paste0("col", seq(ncol(named_mat))) # same as above line

ncol(named_mat)
seq(ncol(named_mat))

rownames(named_mat) <- paste0("row", seq(nrow(named_mat)))

named_mat

named_mat["row1", ] # all element of the 1st row
```
...
::: {.content-smaller .fragment}
Attention: when assignubg new values, you must provide either:
