...
5. Use `data_clean` to draw the graphs you have already seen, *e.g.*: box plots, scatter plots, *etc.*
...
ggplot(data, aes(x = sample_group, y = expression, fill = sample_group)) +
  geom_violin() +
  labs(
...
ggplot(
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) # rotate x-axis' text to avoid overlap
```
